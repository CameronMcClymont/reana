...
REMOTES=$(find -- *.kubeconfig 2>/dev/null | cut -f 1 -d '.' | paste -sd ',' -)
...
apply_resource_files() {
    # Only include node labels if using remote
    local node_labels=""
    if [[ "$CLUSTER_MODE" = "remote" ]]; then
        node_labels="spec:\n  nodeLabels:\n    site: $REMOTE\n"
    fi
...
EOF
    ...
    print_info "Applying LocalQueues..."
    cat <<EOF | kubectl_cmd apply -f -
---
apiVersion: kueue.x-k8s.io/v1beta1
kind: LocalQueue
...
EOF
    ...
    local admission_check_definition_batch=""
    if [[ "$CLUSTER_MODE" = "local" ]]; then
        admission_check_definition_batch="  admissionChecks:
  - batch"
    fi
...
spec:
  ...
  - coveredResources: ["cpu", "memory"]
    ...
    - name: "default"
      ...
      - name: memory
        nominalQuota: 3Gi
$admission_check_definition_batch
EOF
...
EOF
    ...
    if [[ "$CLUSTER_MODE" = "local" ]]; then
        print_info "Applying AdmissionChecks..."
        cat <<EOF | kubectl_cmd apply -f -
---
apiVersion: kueue.x-k8s.io/v1beta1
kind: AdmissionCheck
...
status_kueue() {
    ...
    if [[ "$CLUSTER_MODE" = "local" ]]; then
        ...
        local kubeconfigs
        kubeconfigs=$(find -- *.kubeconfig 2>/dev/null)
        ...
        resources_files=$(find -- *-resources.yaml 2>/dev/null)
...
while [[ $# -gt 0 ]]; do
    ...
    --remote)
        ...
        # As a fallback, use the value of REMOTES but ONLY if it is not a list.
        ...
        if [[ -n "$REMOTE" ]]; then
            ...
            if [[ "$REMOTES" == *","* ]]; then
                print_error "Please specify a name with --remote as you have multiple remotes available."
                print_info "Available remotes: $REMOTES"
                exit 1
            else
                REMOTE_KUBECONFIG="$REMOTES.kubeconfig"
            fi
        fi
